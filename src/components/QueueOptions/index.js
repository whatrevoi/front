...
export function QueueOptionStepper({ queueId, options, updateOptions }) {
  ...
  const getFormData = (option) => {
    ...
    formData.append("finalize", option.finalize || false);
    formData.append("waitTreatment", option.waitTreatment || false);
    ...
    if (option.edition) {
      return (
        <FormControl className={classes.select}>
          ...
          >
            <MenuItem value={"BUTTON_LIST"}>Lista de Botões</MenuItem>
            <MenuItem value={"OPTION_LIST"}>Lista de Opções</MenuItem>
            <MenuItem value={"TEXT_LIST"}>Lista de Textual</MenuItem>
          </Select>
        </FormControl>
      );
    } else {
  ...
  const renderCheckOptions = (option, index) => {
    return (
      <Grid xs={6} item>
      ...
      <Step key={index}>
        ...
        <StepContent>
          ...
          <Grid spacing={2} container>
            ...
            {option.edition &&
              option.children.length === 0 &&
              renderCheckOptions(option, index)}
...
export function QueueOptions({
  ...
  useEffect(() => {
    if (queue.id !== undefined) {
      setOptionType(queue.optionType);
    }
  }, [queue]);
